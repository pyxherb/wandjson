	...
	while (dumpContext.frames.size()) {
		...
		switch (curFrame.state) {
			...
			case DumpState::DumpingObjectEnd: {
				...
				if (++data.prevIterator == v->data.endConst()) {
					WANDJSON_RETURN_IF_FALSE(dumpContext.writer->write("}", 1));
					dumpContext.frames.popBack();
				} else {
					WANDJSON_RETURN_IF_FALSE(dumpContext.writer->write(",", 1));
